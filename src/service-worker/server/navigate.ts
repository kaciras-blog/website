 ...
 * 那么任何请求都必须等到 ServiceWorker 启动后才能发送，保证其能够被拦截。
 *
 * 通过打开导航预载，可以允许 ServiceWorker 未启动时就发送导航请求，与启动过程并行，
 * 待启动完成后再激活拦截事件，使用 FetchEvent.preloadResponse 获取预载的响应。
 *
...
class NavigateRoute implements Route {
	 ...
	 *
	 * 【AppShell 的缓存问题】
	 * Request 默认缓存是不去服务端检查的，而 AppShell 文件名不带 Hash，必须禁用缓存防止无法更新。
