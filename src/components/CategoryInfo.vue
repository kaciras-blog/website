<template>
	<div class="flex vertical margin-vert">
		...
		<div class="flex vertical panel" v-if="category.id">
			...
			<div class="flex center-align between segment" v-for="sub in subCategories">
...
// import selectCategoryDialog from "../component/dialog/SelectCategoryDialog.vue";
...
export default {
	...
	methods: {
		move(sub) {
			this.$showDialog(selectCategoryDialog.name, null, target => move(sub.id, target, false));
		},
		goBack() {
			this.$router.go(-1);
		},
		...
		uploadBackground() {
			...
			api.misc.uploadImageFile().then(url => Vue.set(this.category, "background", url));
		},
		deleteCategory(sub) {
